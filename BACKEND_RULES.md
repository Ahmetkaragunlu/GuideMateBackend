...
| Dashboard ve aylik rehber kazanc projection'lari | Tamamlandi: dashboard `currentMonthEarningsMinor` ve `GET /api/v1/guides/me/earnings/monthly?year=...` ayni `createdAt`/`REVERSED` haric hesap kuralini kullanir. Aylik response `year`, `month`, `netEarningsMinor`, `currencyCode` tasir ve yeniden eskiye siralanir; yeni tablo yoktur |
